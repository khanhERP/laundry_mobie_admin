...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/reports/revenue-by-store", async (req: TenantRequest, res) => {
    try {
      const { startDate, endDate } = req.query;
      const tenantDb = await getTenantDatabase(req);
      const database = tenantDb || db;

      console.log("📊 Fetching revenue by store:", { startDate, endDate });

      // Get all stores with type_user = 0
      const stores = await database
        .select({
          id: storeSettings.id,
          storeName: storeSettings.storeName,
          storeCode: storeSettings.storeCode,
          typeUser: storeSettings.typeUser,
        })
        .from(storeSettings)
        .where(eq(storeSettings.typeUser, 0));

      console.log(`📍 Found ${stores.length} stores with type_user = 0`);
      ...
      const storeRevenues = await Promise.all(
        stores.map(async (store) => {
          // Build store-specific conditions
          const storeConditions = [
            or(eq(orders.status, "paid"), eq(orders.status, "completed")),
            eq(orders.storeCode, store.storeCode), // Filter by storeCode
            ...(whereConditions.length > 0 ? whereConditions : []),
          ];

          const storeOrders = await database
            .select({
              total: orders.total,
              tax: orders.tax,
          ...
          const revenue = storeOrders.reduce((sum, order) => {
            ...
            const orderRevenue = priceIncludeTax
              ? orderTotal - orderTax
              : orderTotal;
      ...
      console.log("✅ Store revenues calculated:", storeRevenues);
      res.json(storeRevenues);
    } catch (error) {
    ...
    async (req: TenantRequest, res) => {
      ...
      try {
        ...
        if (floorFilter && floorFilter !== "all") {
          ...
          console.log(
            "📊 Orders by date range - Total found:",
            ordersQuery.length,
          );
